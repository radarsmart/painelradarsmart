...
const SORT_OPTIONS: Array<{ value: SortType; label: string }> = [
  { value: "best_deals", label: "Melhores ofertas" },
  { value: "top_selling", label: "Mais vendidos" },
];
...
function actionLabel(action: ProductAction) {
  const labels: Record<ProductAction, string> = {
    staged: "Na curadoria",
    published: "Na curadoria",
    skipped: "Ignorado",
    error: "Erro",
    would_stage: "Iria para curadoria",
    would_publish: "Iria para curadoria",
...
function actionClass(action: ProductAction) {
  if (action === "staged" || action === "would_stage" || action === "published" || action === "would_publish") {
...
function configToForm(config: AutomationConfig): ConfigForm {
  return {
    advertiserId: config.advertiserId || DEFAULT_FORM.advertiserId,
...
function formToPayload(form: ConfigForm) {
  return {
    advertiserId: form.advertiserId.trim() || "18879",
...
export default function AwinAutomationPanel() {
  ...
  async function fetchJson<T>(url: string, init?: RequestInit) {
    ...
    if (!response.ok || payload.ok === false) {
      throw new Error(payload.error || "Falha na automacao AWIN.");
    }
  ...
  async function loadConfig() {
    ...
    try {
      const payload = await fetchJson<{ config: AutomationConfig }>("/api/awin/automation/config");
      ...
      await loadCategories(payload.config.advertiserId || DEFAULT_FORM.advertiserId);
    } catch (err) {
      setError(err instanceof Error ? err.message : "Falha ao carregar configuracao AWIN.");
    } finally {
      setLoading("");
    }
  }

  async function loadCategories(advertiserId: string) {
    try {
      const payload = await fetchJson<{ categories?: string[] }>(
        `/api/awin/feed/${encodeURIComponent(advertiserId || "18879")}?page=1`,
      );
  ...
  async function saveConfig(event?: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setFeedback("Configuracao AWIN salva.");
      await loadCategories(payload.config.advertiserId);
    } catch (err) {
      setError(err instanceof Error ? err.message : "Falha ao salvar configuracao AWIN.");
    } finally {
      setLoading("");
    }
  }
  ...
  async function runAutomation(dryRun: boolean) {
    ...
    try {
      const result = await fetchJson<AutomationRunResult>("/api/awin/automation/run", {
        method: "POST",
        body: JSON.stringify({
          ...formToPayload(form),
          dryRun,
      ...
      const wouldStage = result.wouldStage ?? result.wouldPublish ?? 0;
      const staged = result.staged ?? result.published ?? 0;
      setFeedback(
        dryRun
      ...
      setError(err instanceof Error ? err.message : "Falha ao executar automacao AWIN.");
    } finally {
      setLoading("");
    ...
    <div className="min-h-screen flex-1 space-y-8 bg-[#F5F1ED] p-8 pt-6">
      <div>
        ...
        <p className="mt-1 text-sm font-medium text-muted-foreground">
          Configure criterios, rode testes e envie candidatos AWIN para aprovacao na curadoria.
        </p>
      </div>
      ...
      <form onSubmit={(event) => void saveConfig(event)} className="rounded-3xl border border-rs-border bg-white p-6 shadow-sm">
        <div className="mb-5 flex items-center justify-between gap-3">
          <div>
            ...
            <p className="text-sm text-slate-500">
              Default seguro: dry-run ativo, minimo {formatBRL(80)} e limite 5. Nada vai ao site sem aprovacao manual.
            </p>
          </div>
          {form.active ? (
        ...
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          <label className="space-y-2">
            <span className="text-xs font-bold uppercase tracking-wide text-slate-500">Advertiser ID</span>
            <input
        ...
        <div className="mt-5 grid gap-4 md:grid-cols-3">
          <div className="rounded-2xl bg-emerald-50 p-4">
            ...
            <p className="mt-2 text-3xl font-black text-emerald-700">
              {runSummary?.staged ?? runSummary?.published ?? runSummary?.wouldStage ?? runSummary?.wouldPublish ?? 0}
            </p>
          </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="w-full max-w-md rounded-3xl bg-white p-6 shadow-xl">
            ...
            <p className="mt-2 text-sm text-slate-600">
              Isso vai criar ate {form.limit || 5} candidatos na Curadoria Geral com destino sugerido em {SLOT_OPTIONS.find((item) => item.value === form.slotType)?.label}.
